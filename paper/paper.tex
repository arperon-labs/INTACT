...
\date{Preprint v1\\[2pt]{\small DOI:~\href{https://doi.org/10.5281/zenodo.21507499}{10.5281/zenodo.21507499}}}
...
exactly what the soundness harness verifies --- but it is not a Betti number and
...
classes make each extreme \emph{individually necessary} (all four
extreme-mask cells are rendered in Figure~\ref{fig:tightness}). Drop $M_{\max}$
...
mechanism: one two-pixel $\POSS$ pinch disqualifies a segment whose coverage
is 96\%). That collapse is a property of the toy maps, not the
...
are at \url{https://github.com/arperon-labs/INTACT} (Apache-2.0), and the
paper is archived at Zenodo, \href{https://doi.org/10.5281/zenodo.21507499}{doi:10.5281/zenodo.21507499}.
...
\paragraph{Random-mask baseline (registered).} To measure \S\ref{sec:soundness}'s
...
reachable-mask model of \S\ref{sec:soundness} and the harness's own (per-mask
